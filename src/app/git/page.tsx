...
function GitOperations() {
    ...
    // Atomic Progress States
    const [integrationStep, setIntegrationStep] = useState<number | null>(null);
    const [initRemoteUrl, setInitRemoteUrl] = useState('');
    const integrationSteps = [
        { icon: '🛰️', label: 'Pushing Work' },
        { icon: '🧬', label: 'Merging Code' },
        { icon: '☁️', label: 'Synching Remote' },
        { icon: '🧹', label: 'Cleaning Up' }
    ];
    ...
    const handleInitGit = async () => {
        if (!initRemoteUrl) {
            setError('Please provide a GitHub remote URL');
            return;
        }
        setLoading(true);
        setError('');
    ...
    const handleOpenReview = async () => {
        ...
        try {
            const res = await fetch('/api/git/compare', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ projectPath, base: toBranch, head: mergeFrom })
            });
            const data = await res.json();
            if (data.error) throw new Error(data.error);
    ...
    const handleAtomicIntegrate = async () => {
        if (!mergeFrom || !toBranch) return;
        setLoading(true);
        ...
        try {
            ...
            // Steps sequence for rich UX feedback
            setIntegrationStep(1); await wait(1000);
            setIntegrationStep(2); await wait(1500);
            setIntegrationStep(3); await wait(800);
            ...
            const res = await fetch('/api/git/ops', {
                ...
                body: JSON.stringify({
                    projectPath,
                    action: 'atomic-integrate',
                    data: { base: toBranch, head: mergeFrom }
                })
            });
            const data = await res.json();
            if (data.error) throw new Error(data.error);
            ...
            setIsReviewingPR(false);
            if (projectPath) fetchRepoInfo(projectPath);
            setMergeFrom('');
        } catch (err: any) {
            setError(err.message);
        } finally {
    ...
    return (
        <div className="flex-1 flex flex-col p-8 gap-8 overflow-y-auto custom-scrollbar">
            <header className="flex items-center justify-between border-b border-slate-800/50 pb-6 shrink-0">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{selectedProject?.name} • {repoInfo?.isRepo === false ? 'Git Not Initialized' : `Branch: ${toBranch}`}</p>
                    </div>
                </div>
                <div className="flex items-center gap-6">
                    <button
                        ...
                        disabled={loading}
                        className={`bg-slate-900 hover:bg-slate-800 text-indigo-400 px-6 py-3 rounded-2xl border border-slate-800 font-black text-[10px] uppercase tracking-widest flex items-center gap-3 transition-all ${loading ? 'opacity-50 cursor-not-allowed' : 'active:scale-95'}`}
                    >
                        <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : 'group-hover:rotate-180 transition-transform duration-500'}`} />
                        <span>{loading ? 'Fetching...' : 'Pull Latest'}</span>
                    </button>
                </div>
            </header>
            ...
            <div className="flex-1 grid grid-cols-12 gap-8 min-h-0">
                ...
                <div className="col-span-8 flex flex-col gap-6 pb-20">
                    {repoInfo?.isRepo === false ? (
                        <div className="glass-panel p-12 rounded-[40px] z-50 flex flex-col items-center justify-center gap-8 shadow-xl border-indigo-500/20 relative overflow-hidden group">
                            <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/10 -mr-32 -mt-32 rounded-full blur-3xl group-hover:bg-indigo-500/15 transition-colors"></div>

                            <div className="bg-indigo-500/10 p-6 rounded-[32px] border border-indigo-500/20">
                                <Rocket className="w-12 h-12 text-indigo-400 animate-bounce" />
                            </div>

                            <div className="text-center max-w-md">
                                <h2 className="text-2xl font-black text-white tracking-tight uppercase mb-2">Initialization Hub</h2>
                                <p className="text-xs text-slate-500 font-medium leading-relaxed uppercase tracking-wider">
                                    This project is not yet tracked by Git. Connect it to GitHub to start using the workspace features.
                                </p>
                            </div>

                            <div className="w-full max-w-lg flex flex-col gap-4">
                                <div className="relative group">
                                    <label className="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-[0.2em]">GitHub Remote URL</label>
                                    <input
                                        type="text"
                                        placeholder="git@github.com:user/repo.git"
                                        value={initRemoteUrl}
                                        onChange={(e) => setInitRemoteUrl(e.target.value)}
                                        className="w-full bg-slate-950/60 border border-slate-800/50 rounded-2xl px-6 py-5 text-sm outline-none focus:border-indigo-500/50 text-indigo-300 transition-all mt-2 font-mono shadow-inner"
                                    />
                                    <p className="text-[9px] text-slate-600 mt-2 ml-1 italic capitalize">Hint: The system will automatically perform init, main branch set, and initial push.</p>
                                </div>

                                <button
                                    onClick={handleInitGit}
                                    disabled={loading || !initRemoteUrl}
                                    className="w-full bg-indigo-600 hover:bg-indigo-500 text-white px-10 py-5 rounded-[24px] transition-all shadow-xl shadow-indigo-500/20 flex items-center justify-center gap-4 text-xs font-black uppercase tracking-widest active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed group-hover:scale-[1.02]"
                                >
                                    {loading ? <RefreshCw className="w-5 h-5 animate-spin" /> : <Zap className="w-5 h-5 fill-current" />}
                                    Initialize & Sync to GitHub
                                </button>
                            </div>
                        </div>
                    ) : (
                            ...
                            <div className="grid grid-cols-2 gap-4 overflow-y-auto custom-scrollbar pr-2 max-h-[500px]">
                                {repoInfo?.branches?.map((b: string) => (
                                    <button
                                        ...
                                        onClick={() => {
                                            setToBranch(b);
                                            handleGitOp('switch-branch', { name: b });
                                        }}
                                        className={`px-6 py-4 rounded-[20px] text-sm font-bold border transition-all flex items-center justify-between group relative overflow-hidden ${toBranch === b
                                            ? 'bg-indigo-600 border-indigo-400 text-white shadow-xl shadow-indigo-500/40'
                                            : 'bg-slate-900/40 border-slate-800/50 text-slate-500 hover:text-slate-200 hover:border-slate-600 hover:bg-slate-800/60'
                                            }`}
                                    >
                                        <div className="flex items-center gap-3 relative z-10">
                                            <GitBranch className={`w-4 h-4 transition-colors ${toBranch === b ? 'text-white' : 'text-slate-700 group-hover:text-indigo-400'}`} />
                                            <span className="truncate max-w-[150px]">{b}</span>
                                        </div>
                                        {toBranch === b ? (
                                            <div className="flex items-center gap-2 relative z-10">
                                                <span className="text-[9px] font-black uppercase tracking-tighter bg-white/20 px-2 py-0.5 rounded-md">Active</span>
                                ...
                                <div className="flex gap-3 items-end">
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            ...
                                            className="bg-amber-500/10 hover:bg-amber-500 text-amber-500 hover:text-slate-950 px-8 py-4 rounded-2xl border border-amber-500/20 transition-all text-[10px] font-black uppercase tracking-widest shadow-lg active:scale-95 disabled:opacity-50 flex items-center gap-2 grow justify-center"
                                        >
                                            <Zap className="w-3.5 h-3.5 fill-current" /> ⚡ Review & Integrate
                                        </button>
                                        <button
                                            onClick={() => mergeFrom && handleGitOp('merge', { from: mergeFrom })}
                                            disabled={loading || !mergeFrom}
                                            className="bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white px-6 py-4 rounded-2xl border border-slate-700/50 transition-all text-[10px] font-black uppercase tracking-[0.1em] active:scale-95 disabled:opacity-50"
                                        >
                                            Quick Merge
                                        </button>
                                    </div>
                    ...
                    <div className="glass-panel p-8 rounded-[40px] flex flex-col gap-6 shadow-sm border-slate-800/40 relative group">
                        <div className="flex items-center justify-between border-b border-slate-800/50 pb-4 shrink-0">
                            <div className="flex items-center gap-3">
                                <Layers className="w-4 h-4 text-indigo-400" />
                                <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-500">Pending Changes</h3>
                            </div>
                            <button
                                onClick={() => projectPath && fetchRepoInfo(projectPath)}
                                disabled={loading}
                                className="p-2 hover:bg-white/5 rounded-xl transition-all active:scale-95 group"
                                title="Refresh Status"
                            >
                                <RefreshCw className={`w-3.5 h-3.5 text-slate-500 group-hover:text-indigo-400 transition-all ${loading ? 'animate-spin' : ''}`} />
                            </button>
                        </div>
                        <div className="shrink-0">
                            {loading && !integrationStep ? (
                                <div className="space-y-4 py-4 animate-pulse">
                                    {[1, 2, 3].map(i => (
                                        <div key={i} className="h-6 bg-slate-800/50 rounded-lg w-full"></div>
                                    ))}
                                </div>
                ...
                <div className={`col-span-4 flex flex-col gap-8 pb-20 transition-opacity ${repoInfo?.isRepo === false ? 'opacity-30 pointer-events-none' : ''}`}>
                    {/* Delivery Hub */}
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-8 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="glass-panel w-full max-w-6xl max-h-[90vh] rounded-[40px] shadow-2xl border-indigo-500/20 flex flex-col overflow-hidden animate-in slide-in-from-bottom-8 duration-500">
                        <div className="flex items-center justify-between p-8 border-b border-slate-800/50 bg-slate-900/40">
                            <div className="flex items-center gap-6">
                                ...
                                <div className="flex flex-col gap-1">
                                    ...
                                    <h3 className="text-xl font-black text-white tracking-tight">Merge Review</h3>
                                </div>
                            </div>
                            <button
                        ...
                        <div className="p-8 border-t border-slate-800/50 bg-slate-900/40 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest italic opacity-60">Prepare for internal synchronization...</span>
                            </div>
                            <div className="flex items-center gap-4">
                                <button
                                    onClick={() => setIsReviewingPR(false)}
                                ...
                                <button
                                    onClick={handleAtomicIntegrate}
                                    disabled={loading}
                                    className="bg-amber-500 hover:bg-amber-400 text-slate-950 px-10 py-4 rounded-2xl transition-all shadow-lg shadow-amber-500/20 text-[10px] font-black uppercase tracking-widest active:scale-95 flex items-center gap-2"
                                >
                                    {loading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Zap className="w-4 h-4 fill-current" />}
                                    Atomic Integrate & Close
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            {integrationStep !== null && (
                <div className="fixed inset-0 z-[300] flex items-center justify-center bg-slate-950/90 backdrop-blur-xl animate-in fade-in duration-500">
                    <div className="flex flex-col items-center gap-12 max-w-md w-full px-10">
                        <div className="relative">
                            <div className="absolute inset-0 bg-indigo-500/20 blur-3xl rounded-full scale-150 animate-pulse"></div>
                            <RefreshCw className="w-20 h-20 text-indigo-500 animate-spin relative" />
                        </div>

                        <div className="flex flex-col items-center gap-4 text-center">
                            <h3 className="text-2xl font-black text-white tracking-tight uppercase">Atomic Synchronizing</h3>
                            <p className="text-slate-500 text-sm font-medium">Executing multi-step integration cycle...</p>
                        </div>
                        ...
                        <div className="w-full space-y-3">
                            {integrationSteps.map((step, idx) => (
                                <div
                                    key={idx}
                                    className={`flex items-center gap-4 p-4 rounded-3xl border transition-all duration-500 ${integrationStep > idx ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' :
                                        integrationStep === idx ? 'bg-indigo-500/10 border-indigo-500/40 text-white shadow-lg shadow-indigo-500/10 scale-105' :
                                            'bg-slate-900/40 border-slate-800/50 text-slate-700 opacity-40'
                                        }`}
                                >
                                    <span className="text-xl">{integrationStep > idx ? '✅' : step.icon}</span>
                                    <span className="text-[10px] font-black uppercase tracking-widest">{step.label}</span>
                                    {integrationStep === idx && <RefreshCw className="w-3 h-3 animate-spin ml-auto opacity-50" />}
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="w-full bg-slate-900 h-1.5 rounded-full overflow-hidden border border-slate-800/50">
                            <div
                                ...
                                style={{ width: `${((integrationStep + 1) / integrationSteps.length) * 100}%` }}
                            ></div>
                        </div>
                    </div>
                </div>
